...
&\approx \frac{1}{N} \sum_{i=1}^N G(\tau_i) \nabla_\theta \log P(\tau_i|\theta), \quad \tau_i \sim P(\tau|\theta)
\end{aligned}
$$
...
$$
L(\theta) = -\frac{1}{N} \sum_{i=1}^N G(\tau_i) \log P(\tau_i|\theta), \quad \tau_i \sim P(\tau|\theta)
$$
...
继续我们之前的推导，我们发现当我们将目光聚集到单个时间步时候，上述的 Loss 函数中的 $G(\tau_i)$ 可以被替换为 $G_t$，即当前时间步的回报。此时我们可以得到以下的损失函数：

$$
L(\theta) = -\frac{1}{N} \sum_{i=1}^N G_t \log P(a_t|\theta), \quad a_t \sim P(a_t|\theta)
$$

此时注意到 $G_t$ 的方差可能会比较大，因此我们可以引入一个基线（Baseline）来降低方差。这样我们就可以得到以下的损失函数：

$$
L(\theta) = -\frac{1}{N} \sum_{i=1}^N (G_t - B^\pi(s_t)) \log P(a_t|\theta), \quad a_t \sim P(a_t|\theta)
$$

### 价值函数改写

进一步的，我们可以用价值函数改写上述损失函数，引入之前得到的优势价值函数 $A^\pi(s, a)$，从而得到以下的损失函数：

$$
L(\theta) = -\frac{1}{N} \sum_{i=1}^N A^\pi(s_t, a_t) \log P(a_t|\theta), \quad a_t \sim P(a_t|\theta)
$$
...
\log P(\tau|\theta) &= \sum_t \log P(a_t|s_t)
\end{aligned}
$$
...
$$
L(\theta) = -\frac{1}{N} \sum_{i=1}^N \sum_t A^\pi(s_t, a_t) \log P(a_t|s_t), \quad a_t \sim P(a_t|\theta)
$$
...
于是我们可以将 Actor-Critic 方法的损失函数改写为：

$$
L(\theta) = -\frac{1}{N} \sum_{i=1}^N \sum_t A^{GAE}_\lambda(s_t, a_t) \log P(a_t|s_t), \quad a_t \sim P(a_t|\theta)
$$
...
$$
D_{KL}(\pi_\theta || \pi_{\theta'}) = \sum_a P(a|s_t) \log \frac{P(a|s_t)}{P'(a|s_t)}
$$
...
这种情况下，PPO 的损失函数可以定义为：

$$
L(\theta') = -\frac{1}{N} \sum_{i=1}^N \sum_t \min \left( A^{GAE}_\lambda(s_t, a_t) \frac{P'(a_t|s_t)}{P(a_t|s_t)}, \text{clip}\left( \frac{P'(a_t|s_t)}{P(a_t|s_t)}, 1 - \epsilon, 1 + \epsilon \right) A^{GAE}_\lambda(s_t, a_t) \right)
$$
